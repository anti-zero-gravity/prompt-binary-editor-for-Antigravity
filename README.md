...
- **スマート検索** — ASCII文字列の検索に加え、`%s` や `%d` などの**書式指定子（Verb）を含む文字列の曖昧マッチング（N-gram検索）**に対応。プロンプト内の変数部分を自動的に無視して位置を特定できます。
- **行マッピング** — 外部のプロンプトシート（Markdown等）の行テキストから、バイナリ内の対応するフラグメントを自動検出。(クリックで検索/CTRL+クリックで検索窓へ挿入）
)
- **インライン編集** — 検索窓からの直接書き換え、Deleteキーによるスペース埋め。Undo/Redo完備。
- **自動埋めたて** —　読み込まれたプロンプトシートのASCII行を全て、Shift+Deleteキーでスペース埋め。完全一致検索→N-gram検索の2段階処理。
